...
class SpectrogramDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if torch.is_tensor(idx):
            ...
            X = torch.stack(list(map(lambda audio_path: torchaudio.transforms.Spectrogram()(_padding_cropping(torchaudio.load(audio_path)[0], self.padding_cropping_size)) if self.padding_cropping_size is not None 
                                     else torchaudio.transforms.Spectrogram()(torchaudio.load(audio_path)[0]), self.wav_path_label_df.iloc[idx, 0])))
            y = torch.tensor(self.wav_path_label_df.iloc[idx, 1].tolist())
